---
title: "CRISIS_Factor_Pipeline EFA RRB Sample 1 3/25"
...
All_Site_Split1$repetitivemotor <- as.numeric(All_Site_Split1$repetitivemotor)
All_Site_Split1$sensoryseeking <- as.numeric(All_Site_Split1$sensoryseeking)
All_Site_Split1$ritualsroutines <- as.numeric(All_Site_Split1$ritualsroutines)
All_Site_Split1$adjusttochange <- as.numeric(All_Site_Split1$adjusttochange)
All_Site_Split1$familyroutinechange <- as.numeric(All_Site_Split1$familyroutinechange)
All_Site_Split1$restrictedinterestactivity <- as.numeric(All_Site_Split1$restrictedinterestactivity)


#selecting the items for the factor to be analyzed using EFA (RRB items adjust to change is reverse coded):
domain_data <- All_Site_Split1 %>% select(repetitivemotor, sensoryseeking, ritualsroutines, adjusttochange, familyroutinechange, restrictedinterestactivity)
